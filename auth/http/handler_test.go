...
func TestCookieSameSiteOverride(t *testing.T) {
	now := time.Now().UTC()
	manager := issuingManager(now)
	opts := testOptions(stubAuthenticator("admin", "secret", "user-1"))
	...
	if got := refresh.SameSite; got != http.SameSiteLaxMode {
		t.Fatalf("expected refresh cookie SameSite=%v, got %v", http.SameSiteLaxMode, got)
	}
	csrf := cookieByName(rec.Result().Cookies(), authsession.DefaultCSRFCookieName)
	if csrf == nil {
		t.Fatal("expected csrf cookie")
	}
	if got := csrf.SameSite; got != http.SameSiteLaxMode {
		t.Fatalf("expected csrf cookie SameSite=%v, got %v", http.SameSiteLaxMode, got)
	}
}
...
func TestRefresh(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		r := mustNewTestRouter(t, manager, testOptions(mustNewStaticPassword(t, "dashboard-admin", "secret")))
